...
import it.pagopa.interop.signalhub.pull.service.auth.*;
...
class JWTFilterTest {
    ...
    void filterOnJWTExceptionTest() {
        ...
        WebFilterChain filterChain = filterExchange -> Mono.empty();
        Mockito.when(jwtConverter.apply(Mockito.any()))
                .thenThrow(new JWTException(JWT_NOT_VALID, JWT_NOT_VALID.getMessage(), HttpStatus.FORBIDDEN, jwt.getToken()));

        JWTCache jwtCacheMono = Mockito.mock(JWTCache.class);
        Mockito.when(jwtService.saveOnCache(Mockito.any()))
                .thenReturn(Mono.just(jwtCacheMono));
    ...
    @Test
    void filterWhithoutAuthorizationTest() {
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest
                        .get("/your-url"));
